    ...
    module Pubweave
      class ArticlesController < ApplicationController
        ...
        def deny_published_article_update
          render json: { error: 'You cannot edit a published article.' }, status: :forbidden and return if @article.status == 'published' && !@current_user.is_a?(Admin)
        end
        ...
        def permit_table_data(whitelist)
          ...
          whitelist[:content][:blocks].each_with_index do |block, index|
            if block[:type] == 'table' && params[:article][:content][:blocks][index][:data].present?
